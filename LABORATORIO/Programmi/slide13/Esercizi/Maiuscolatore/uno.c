...
/* Un Server per rendere maiuscole linee di testo */
...
int main(int argc, char ** argv) {
    ...
    while (1) {
        ...
        process = fork();
        if(process == 0){
            printf("azione elaborata dal proccesso: %d\n",getpid());
            fprintf(stdout, "Aperta connessione.\n");
            // il server gli da il benvvenuto al client
            send(fd, "\nBenvenuto nel Maiuscolatore!\n", LINESIZE, 0);
            // elabora l'input e glie lo invia tramite la funzione upperlines
            upperlines(fd, fd);
            // chiude il canale
            close(fd);
